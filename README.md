...
bun setup
```
4. Configure your environment variables in `server/.env` (see [Environment Variables](#environment-variables) section below)
5. Start Autumn with Docker (includes PostgreSQL + automatic migrations):
```bash
docker compose -f docker-compose.dev.yml up -d
```
6. Add the Platfrom account webhook to your Stripe dashboard:
```
cd server
bun run test.ts // for the sandbox environment
```
OR
```
cd server
bun run register.ts // for the production environment
```
...
```
4. Configure your environment variables in `server/.env` (see [Environment Variables](#environment-variables) section below)
5. Generate the relevant tables in your postgres DB
```bash
...
Before running Autumn, you need to configure the following environment variables in your `server/.env` file. The setup script (`bun setup`) will auto-generate most of these values for you:

### Essential Environment Variables

```env
# Auth - Auto-generated by setup script
BETTER_AUTH_SECRET=<auto_generated>
BETTER_AUTH_URL=<auto_generated>
CLIENT_URL=<auto_generated>

# Stripe - Auto-generated by setup script
LOCALTUNNEL_RESERVED_KEY=<auto_generated>
ENCRYPTION_IV=<auto_generated>
ENCRYPTION_PASSWORD=<auto_generated>
STRIPE_WEBHOOK_URL=<auto_generated>

# Database - Only required for local development without Docker
DATABASE_URL=postgresql://postgres:Test123@localhost:5432/autumn

# Stripe Sandbox Keys - You need to provide these from your Stripe dashboard
...
4. For webhook secrets, go to **Developers** → **Webhooks** and copy the signing secret from your webhook endpoint

> ⚠️ **Important**: Make sure to use your **test/sandbox** keys during development, not your live keys.
...
> ℹ️ Our set up script initializes the required env vars and (optionally) a Supabase instance. With Docker (Option 1), you get a containerized PostgreSQL automatically. For manual setup (Option 2), you can use your own Postgres instance by setting the `DATABASE_URL` in `server/.env` (see [Environment Variables](#environment-variables) section above)
